...
export const metadata: Metadata = { title: "Desarrollo del colaborador" };

async function loadDevelopmentRecord(employeeId: string) {
  try {
    return await getDevelopmentRecordForAdministration(employeeId);
  } catch (error) {
    if (error instanceof DevelopmentDomainError && error.code === "employee_not_found")
      notFound();
    throw error;
  }
}

export default async function CollaboratorDevelopmentPage({
  params,
}: {
